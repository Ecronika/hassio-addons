...
# --- NEU: KNX-Medium fuer USB + RF-Domaenenadresse ---------------------------
# usb_medium: tp1 (default) | rf | pl110
USB_MEDIUM=$(bashio::config 'usb_medium')
if bashio::config.is_empty 'usb_medium'; then
    USB_MEDIUM="tp1"
fi
# rf_domain_address: 6-Byte-Hexwert ohne Trenner (nur bei rf/pl110 noetig)
ADD_DOMAIN_ADDRESS=""
if ! bashio::config.is_empty 'rf_domain_address'; then
    ADD_DOMAIN_ADDRESS=" domainAddress=\"$(bashio::config 'rf_domain_address')\""
fi
# ----------------------------------------------------------------------------
...
CONFIG_XML="$CONFIG_XML
		<knxSubnet type=\"usb\" medium=\"$USB_MEDIUM\"$ADD_DOMAIN_ADDRESS$ADD_KNX_SOURCE_OVERRIDE>$USB_DEVICE</knxSubnet>"
fi
CONFIG_XML="$CONFIG_XML
